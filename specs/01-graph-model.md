...
- **value** — emitted on entries in `reactiveValues(name = ...)` and on subsequent named writes (`rv$name <- ...`). Best-effort by name: **every** read resolved this way carries SVT-W003, because name-only resolution is an inference rather than a proof and an auditor needs to see which dependencies were inferred. A read whose key is not statically resolvable produces no node at all and is SVT-W001 territory instead.
...
- **SVT-W002** — Runtime-UI source for input definition. Emitted when an input-creating UI call (`*Input(...)`, `actionButton`, `actionLink`, `radioButtons`, `checkboxGroupInput`, `submitButton`, …) with a **literal** id appears inside a runtime-UI call: `renderUI()`, `insertUI()`, `appendTab()`, `prependTab()` or `insertTab()`. All five insert UI after the app has started, so an input born in one has no static definition site and any test that sets it must render that UI first. Detection is deliberately narrow: a non-literal id inside a render context is already SVT-W001 territory, and a UI call outside one is a normal static input
