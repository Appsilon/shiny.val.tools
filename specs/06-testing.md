...
- **internals** — `reactive` and `value` nodes strictly between stimuli and roots. Each carries `observable_via`, which is always `direct`: inside `testServer()` a named reactive is callable by name (`my_reactive()`), so internals need no plumbing at all. That is why the package generates nothing else, and why the field is present at all — it records that no export machinery was required.

  Opacity is an **observable** property, never an internal one (see the opacity rule below). An internal is by definition a named reactive and a named reactive is always callable; what a *root* hands back through a render function is a different question. SVT-W312 therefore attaches to observables only.
...
- **blockers** — any of `SVT-W001` (dynamic input id), `SVT-W002` (`renderUI`-created input), `SVT-W003` (name-only `reactiveValues` resolution), `SVT-W009` (metaprogramming) attached to a node in the closure. Their presence means the stimulus list is incomplete and a test built from it can pass while missing a code path. Surfaced as SVT-W308 on the surface record, not silently swallowed.
