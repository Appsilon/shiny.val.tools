...
Phases 1–2 are implemented: SVT-W303, W308, W309 and W312 are emitted
today. The remaining codes are allocated and specified but not yet
raised — test discovery, coverage classification, traceability and
result ingestion (phases 3–4) are not built.

| Code      | Name                                                  | Triggered by | Emitted today |
| --------- | ----------------------------------------------------- | ------------ | ------------- |
| SVT-W301  | Feature or module has no mapped test                  | no test maps to the subgraph (verification gap) | no |
| SVT-W302  | Partial coverage                                      | some observables or stimuli are unexercised | no |
| SVT-W303  | Scaffold present but not filled in                    | a generated scaffold still carries its `skip()` marker | yes |
| SVT-W304  | Orphan test                                           | a test maps to no feature, module, or helper | no |
| SVT-W305  | Unknown `@covers` target                              | an annotation or manifest `tests:` entry names something not in the graph | no |
| SVT-W306  | *(retired, never emitted)*                            | allocated when the design still generated `shinytest2` scaffolds; replaced by SVT-W312 | never |
| SVT-W307  | Test surface changed since covering test               | `surface_hash` moved while every covering test file's md5 did not | no |
| SVT-W308  | Test surface incomplete                               | a static-analysis blocker (W001/W002/W003/W009) sits in the subgraph | yes |
| SVT-W309  | Scaffold target already exists                        | `target = "app"` would overwrite an existing test file; skipped instead | yes |
| SVT-W310  | Mapped test failed or absent from results             | an ingested CI report reports a failure, or has no entry for a mapped test | no |
| SVT-W311  | `verification = not_required` without rationale       | a waiver with no `rationale_verification` | no |
| SVT-W312  | Observable is opaque under `testServer()`             | a root's `def_call` is `renderPlot`, `renderUI`, `downloadHandler`, ... | yes |
